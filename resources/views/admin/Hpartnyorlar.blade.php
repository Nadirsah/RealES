    ...
    <body class="sb-nav-fixed">
       @include('admin.navbar')
        ...
        @include('admin.sidebar')
            ...
            <form action="{{route('Storehpartnyorlar')}}" method="POST" enctype="multipart/form-data">
                @csrf
  <div class="mb-3">
...
<table class="table">
  ...
  @foreach($data as $baza)
    <tr>
      <th>{{$baza->id}}</th>
      <td>{{$baza->image}}</td>
      <td><a href="{{'edithpartnyorlar/'.$baza['id']}}"><i class="fa-solid fa-pen-to-square btn btn-info" ></i></a>
      <a href="{{'deletehpartnyorlar/'.$baza['id']}}"><i class="fa-solid fa-trash  btn btn-danger" ></i></a>
