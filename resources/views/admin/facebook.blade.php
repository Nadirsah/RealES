    ...
    <body class="sb-nav-fixed">
       @include('admin.navbar')
        ...
        @include('admin.sidebar')
            ...
            <form action="{{route('storefacebook')}}" method="POST" enctype="multipart/form-data">
                @csrf
  <div class="mb-3">
    <label for="exampleInputEmail1" class="form-label">Url</label>
    <input type="url" name="url" class="form-control bg-warning" >
...
<table class="table">
  ...
  @foreach($data as $baza)
    <tr>
      <th>{{$baza->id}}</th>
      <td>{{$baza->url}}</td>
      <td><a href="{{'editfacebook/'.$baza['id']}}"><i class="fa-solid fa-pen-to-square btn btn-info" ></i></a>
      <a href="{{'deletefacebook/'.$baza['id']}}"><i class="fa-solid fa-trash  btn btn-danger" ></i></a>
